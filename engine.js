...
(function (root, factory) {
  ...
  var ALGORITHM_VERSION = 'DE-1.2.1';
  ...
  /* shared: trim a picked list so the mandatory minimums fit, then measure it */
  function buildCandidate(pick, targetPi) {
    pick = pick.slice();
  ...
  function drawCandidate(pool, targetPi, rand) {
    ...
    return buildCandidate(weightedDraw(pool, k, rand), targetPi);
  }
  ...
  /* Collect every VALID, fully-allocated candidate, then pick ONE at random from
     the near-best pool:   remaining <= bestRemaining + targetPi * marginPct
     An over-budget candidate can never enter the pool.
     If an EXACT result exists (bestRemaining === 0) the pool is restricted to the
     exact candidates — a worse result is never chosen over a mathematically exact one. */
  function searchBest(pool, targetPi, rand, attempts, marginPct) {
    var cands = [];

    function consider(cand) {
      if (!cand) return;
      var res = allocate(cand, targetPi, rand);
      if (res.spentPi > targetPi) return;                             // never accept an overrun
      cands.push({ sel: cand, res: res, remaining: targetPi - res.spentPi });
    }

    /* GUARANTEED CANDIDATE — the full eligible pool is always evaluated once.
       Without this the random draw can miss k = pool.length entirely and return
       a strictly worse subset. */
    consider(buildCandidate(pool, targetPi));

    var n = attempts || 200;
    for (var a = 0; a < n; a++) consider(drawCandidate(pool, targetPi, rand));

    if (!cands.length) return null;

    var bestRemaining = cands[0].remaining;
    for (var i = 1; i < cands.length; i++)
      if (cands[i].remaining < bestRemaining) bestRemaining = cands[i].remaining;

    var mp = (marginPct === undefined || marginPct === null || isNaN(marginPct)) ? 0.005 : Number(marginPct);
    var cutoff = (bestRemaining === 0) ? 0 : bestRemaining + Math.floor(targetPi * mp);

    var near = cands.filter(function (c) { return c.remaining <= cutoff; });
    var chosen = near[Math.floor(rand() * near.length)] || near[0];

    chosen.diag = {
      bestRemaining: E(bestRemaining),
      nearBestCandidateCount: near.length,
      bestMarginPct: mp,
  ...
  function generate(input) {
    ...
    var attempts = Number(input.attempts) ||
                   Number((input.settings || {}).DISTRIBUTION_ATTEMPTS) || 200;
    var marginRaw = (input.settings || {}).DISTRIBUTION_BEST_MARGIN_PCT;
    var marginPct = (marginRaw === undefined || marginRaw === null || marginRaw === '' || isNaN(Number(marginRaw)))
                    ? 0.005 : Number(marginRaw);
    var best = searchBest(pool, targetPi, rand, attempts, marginPct);
    ...
    return {
      run: {
        ...
        randomSeed: seed, algorithmVersion: ALGORITHM_VERSION,
        algorithmAttempts: attempts, fullPoolEvaluated: true,
        bestRemaining: best.diag.bestRemaining,
        nearBestCandidateCount: best.diag.nearBestCandidateCount,
        bestMarginPct: best.diag.bestMarginPct,
        candidatesEvaluated: best.diag.candidatesEvaluated,
